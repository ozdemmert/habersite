...
// Get story news
$storyNews = [];
if (!empty($sections['story1']))
    $storyNews[] = $newsObj->getById($sections['story1']);
if (!empty($sections['story2']))
    $storyNews[] = $newsObj->getById($sections['story2']);
if (!empty($sections['story3']))
    $storyNews[] = $newsObj->getById($sections['story3']);
...
<body class="bg-white">
    ...
    <div id="headerPlaceholder" class="hidden w-full pointer-events-none transition-all duration-300"></div>
    <main class="min-h-screen py-4 w-[1080px] m-auto">
                    ...
                    <div class="relative h-[400px] overflow-hidden group rounded-lg shadow-md">
                        <a href="news.php?id=<?php echo $news['id']; ?>">
                            ...
                            <div class="absolute top-3 left-3">
                                <span
                                    class="category-badge accent-bg"><?php echo htmlspecialchars($news['category']); ?></span>
                            </div>
                            <div class="absolute bottom-0 left-0 p-4 w-full text-white">
                                <h2
                                    class="text-base font-bold line-clamp-3 group-hover:text-[#f39200] transition duration-200">
            ...
            <?php if ($mainNews1): ?>
                <div class="mb-16 border-t border-gray-200 pt-8">
                    <div class="flex flex-col">
                        <span
                            class="text-xs font-semibold accent-color uppercase"><?php echo htmlspecialchars($mainNews1['category']); ?></span>
                        <h1 class="text-2xl font-bold mt-1 mb-2 primary-color">
                            <a href="news.php?id=<?php echo $mainNews1['id']; ?>"
                                class="hover:accent-color transition-colors duration-200">
                                <?php echo htmlspecialchars($mainNews1['title']); ?>
                            </a>
                        </h1>
                        <p class="text-sm text-gray-700 mb-4">
                            <?php echo htmlspecialchars(isset($mainNews1['summary']) ? $mainNews1['summary'] : createSummary($mainNews1['content'])); ?>
                        </p>
                    ...
                    <?php if ($mainNews2): ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                            <div
                                class="border border-gray-200 p-4 text-center rounded-lg hover:shadow-md transition-all duration-300">
                                <a href="news.php?id=<?php echo $mainNews2['id']; ?>"
                                    class="hover:accent-color transition-colors duration-200">
                                    <p class="text-sm font-medium primary-color">
                                        <?php echo htmlspecialchars($mainNews2['title']); ?>
                                    </p>
                                    <p class="text-xs accent-color mt-1"><?php echo htmlspecialchars($mainNews2['category']); ?>
                                        hakkında daha fazla</p>
                                </a>
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-100">
                        <div class="md:flex">
                            ...
                            <div class="p-6 md:w-2/3">
                                <h2 class="text-2xl font-bold mb-2 primary-color">
                                    <?php echo htmlspecialchars($latestPortre['first_name'] . ' ' . $latestPortre['lastname']); ?>
                                </h2>
                                <div class="text-sm text-gray-600 mb-4">
                                    <?php if (!empty($latestPortre['degree'])): ?>
                                        <span
                                            class="mr-3 font-medium accent-color"><?php echo htmlspecialchars($latestPortre['degree']); ?></span>
                                    <?php endif; ?>
                                ...
                                <p class="text-gray-700 mb-4">
                                    <?php echo htmlspecialchars(createSummary($latestPortre['biography'], 300)); ?>
                                </p>
                                <a href="portre_detay.php?id=<?php echo $latestPortre['id']; ?>"
                                    class="inline-block px-4 py-2 primary-bg text-white font-medium rounded hover:bg-opacity-90 transition-colors">
                                    Devamını Oku
                                </a>
                            </div>
                        </div>
            ...
            <?php if ($video_id): ?>
                <div class="mb-12 border-t border-gray-200 pt-8">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <span class="text-xs text-white accent-bg px-2 py-1 rounded-md"><i
                                class="fab fa-youtube mr-1"></i>YouTube</span>
                    </div>
                    <div class="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-100">
                        ...
                        <div class="p-4 border-t border-gray-100">
                            <p class="text-gray-500 text-sm accent-color font-medium">En güncel ve öne çıkan video içerikler
                                burada</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="mb-16 border-t border-gray-200 pt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <?php foreach ($featuredNews as $news): ?>
                        <div
                            class="bg-white shadow rounded overflow-hidden transition-all duration-300 hover:shadow-lg border border-gray-100">
                            <a href="news.php?id=<?php echo $news['id']; ?>">
                                <div class="relative">
                                    ...
                                    <div class="absolute top-2 left-2">
                                        <span
                                            class="category-badge primary-bg"><?php echo htmlspecialchars($news['category']); ?></span>
                                    </div>
                                </div>
                                <div class="p-4">
                                    <h3
                                        class="font-bold mb-2 line-clamp-2 primary-color hover:accent-color transition-colors">
                                        <?php echo htmlspecialchars($news['title']); ?>
                                    </h3>
                                    <p class="text-sm text-gray-600 line-clamp-3 mb-3">
                                        <?php echo htmlspecialchars(isset($news['summary']) ? $news['summary'] : createSummary($news['content'])); ?>
                                    </p>
                                    <div class="flex justify-between text-xs text-gray-500">
                                        <span
                                            class="accent-color font-medium"><?php echo date('d.m.Y', strtotime($news['created_at'])); ?></span>
                                    </div>
                                </div>
                            </a>
                        </div>
