...
$storyMapObj = new StoryMap();
$categoryObj = new Category();

// Get featured items first - independent of category
$sql = "SELECT * FROM storymaps WHERE published = 1 AND is_featured = 1 ORDER BY created_at DESC LIMIT 3";
...
// StoryMaps içeriklerini çekme
if ($active_category) {
    $all_storymaps = [];
    $storymaps_ids = [];
    $sql = "SELECT storymaps_id FROM storymaps_category WHERE category_id = $active_category";
    $result = mysqli_query($conn, $sql);
    while ($row = mysqli_fetch_assoc($result)) {
        $storymaps_ids[] = $row['storymaps_id'];
    }
    if (!empty($storymaps_ids)) {
        $ids = implode(',', $storymaps_ids);
        // Exclude featured items from main grid
        $sql = "SELECT s.*, c.name as category_name 
                FROM storymaps s
                LEFT JOIN storymaps_category sc ON s.id = sc.storymaps_id
                LEFT JOIN categories c ON sc.category_id = c.id
                WHERE s.id IN ($ids) AND s.published = 1 AND s.is_featured = 0 
                GROUP BY s.id
                ORDER BY s.created_at DESC 
                LIMIT $per_page OFFSET $offset";
        $storymaps_result = mysqli_query($conn, $sql);
        $all_storymaps = mysqli_fetch_all($storymaps_result, MYSQLI_ASSOC);
    ...
    $sql = "SELECT s.*, c.name as category_name 
            ...
            LEFT JOIN storymaps_category sc ON s.id = sc.storymaps_id
            LEFT JOIN categories c ON sc.category_id = c.id
            WHERE s.published = 1 AND s.is_featured = 0 
            GROUP BY s.id
            ORDER BY s.created_at DESC 
            LIMIT $per_page OFFSET $offset";
    $storymaps_result = mysqli_query($conn, $sql);
...
// Get category info for featured items
if (!empty($featured_items)) {
    foreach ($featured_items as $key => $item) {
        $sql = "SELECT c.name FROM categories c 
                ...
                WHERE sc.storymaps_id = {$item['id']} LIMIT 1";
        $cat_result = mysqli_query($conn, $sql);
        $category = mysqli_fetch_assoc($cat_result);
        if ($category) {
            $featured_items[$key]['category'] = $category['name'];
        }
    }
}
...
<head>
    ...
    <title>StoryMaps Projeleri</title>
...
<?php require_once 'templates/header.php'; ?>
    <main class="min-h-screen">
                ...
                <div class="flex flex-col space-y-5">
                    ...
                    <div class="featured-card rounded-xl overflow-hidden flex">
                        ...
                        <div class="w-2/3 p-6 flex flex-col justify-between">
                            ...
                            <div>
                                <a href="storymaps_detay.php?id=<?php echo $featured['id']; ?>" class="featured-read-more">
        ...
        <div class="max-w-6xl mx-auto px-4">
            <!-- Sayfa Başlığı -->
            <h1 class="text-3xl font-bold mb-6 border-b border-[#f39200] pb-2 inline-block text-[#022d5a]">StoryMaps Projeleri</h1>
            
            <!-- Kategori Sekmeleri -->
            <div class="flex flex-wrap gap-2 mb-8">
                <a href="storymaps.php" class="px-4 py-1 rounded-full text-sm font-medium <?php echo $active_category == 0 ? 'bg-[#022d5a] text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-100'; ?>">Tümü</a>
                <?php foreach($categories as $cat): ?>
                    <a href="storymaps.php?cat=<?php echo $cat['id']; ?>" class="px-4 py-1 rounded-full text-sm font-medium <?php echo isActiveCat($cat['id'], $active_category); ?>"><?php echo htmlspecialchars($cat['name']); ?></a>
                <?php endforeach; ?>
            </div>
            
            <!-- İçerik Grid -->
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                <?php if (!empty($all_storymaps)): foreach($all_storymaps as $storymap): ?>
                <div class="bg-white shadow rounded-lg overflow-hidden hover:shadow-md transition duration-200">
                    <a href="storymaps_detay.php?id=<?php echo $storymap['id']; ?>" class="block">
                        <div class="relative h-48 overflow-hidden">
                            <?php if (!empty($storymap['featured_image'])): ?>
                                <img src="<?php echo $storymap['featured_image']; ?>" 
                                     alt="<?php echo htmlspecialchars($storymap['title']); ?>" 
                                     class="w-full h-full object-cover">
                            <?php else: ?>
                                <div class="bg-gray-100 w-full h-full flex items-center justify-center text-gray-400">
                                    <span>Görsel Yok</span>
                            ...
                            <span class="absolute top-3 left-3 px-3 py-1 text-xs font-bold rounded bg-[#022d5a] text-white">
                                <?php echo htmlspecialchars(isset($storymap['category_name']) ? $storymap['category_name'] : $storymap['category']); ?>
                            </span>
                        </div>
                        <div class="p-4">
                            <div class="text-xs text-[#f39200] font-medium mb-1"><?php echo date('d F Y', strtotime($storymap['created_at'])); ?></div>
                            <h2 class="text-lg font-semibold mb-2 line-clamp-2 text-[#022d5a]"><?php echo htmlspecialchars($storymap['title']); ?></h2>
                            <?php if (isset($storymap['content'])): ?>
                            <p class="text-sm text-gray-600 mb-3 line-clamp-3">
                                <?php echo mb_substr(strip_tags($storymap['content']), 0, 150) . '...'; ?>
                            </p>
                            <?php endif; ?>
                ...
                <?php endforeach; elseif (empty($all_storymaps)): ?>
